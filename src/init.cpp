...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: minnscoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: minnscoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizefscamount=<n> " + _("Keep N MinnsCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MinnsCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MinnsCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MinnsCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MinnsCoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MinnsCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MinnsCoin to complete") << "\n";
    ...
    nAnonymizeMinnsCoinAmount = GetArg("-anonymizefscamount", 0);
    if(nAnonymizeMinnsCoinAmount > 999999) nAnonymizeMinnsCoinAmount = 999999;
    if(nAnonymizeMinnsCoinAmount < 2) nAnonymizeMinnsCoinAmount = 2;
    ...
    LogPrintf("Anonymize MinnsCoin Amount %d\n", nAnonymizeMinnsCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1MIN+1000 == (.1MIN+100)*10
       10MIN+10000 == (1MIN+1000)*10
    */
